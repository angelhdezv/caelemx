...
try {
 const data = await getInvitation('bordado', { source: document.body.dataset.source });
 const { event, media, story, dressCode, gifts, invitation, rsvp } = data;
 const copy = data.presentation?.bordado || {};
 ...
 root.innerHTML = `
  ...
  <div class="celebration-layout"><section class="itinerary" aria-labelledby="itinerary-title"><h2 id="itinerary-title">${escape(copy.itineraryTitle || 'Así celebraremos')}</h2><div class="itinerary-events"><div class="itinerary-thread" aria-hidden="true"><span></span></div>${event.schedule.map(item => `<article><span class="thread-flower" aria-hidden="true">✳</span><p class="eyebrow">${escape(item.label)}</p><h2>${escape(item.title)}</h2><p><time datetime="${escape(item.startsAt)}">${escape(format(item.startsAt, { dateStyle: 'medium', timeStyle: 'short' }))}</time><br>${escape(item.venue.name)}</p><p class="address">${escape(item.venue.address)}</p><a class="stitched-link" href="${safeURL(item.venue.mapsUrl)}" target="_blank" rel="noopener noreferrer">Ver ubicación <span aria-hidden="true">↗</span></a></article>`).join('')}</div></section>
  <section class="details" aria-labelledby="details-title"><h2 id="details-title">${escape(copy.detailsTitle || 'Los pequeños detalles')}</h2>
   <div class="detail-item"><h3><button class="detail-toggle" aria-expanded="false" aria-controls="attire-content">Cómo vestir<small>${escape(dressCode.title)}</small><span aria-hidden="true">+</span></button></h3><figure class="attire-preview">${photo(dressCode.illustration)}</figure><div class="detail-panel" id="attire-content" inert><div><ul>${dressCode.guidelines.map(line => `<li>${escape(line)}</li>`).join('')}</ul><p>${escape(dressCode.note)}</p></div></div></div>
  ...
  </div></section></div><div class="signature" aria-hidden="true"><div class="signature-embroidery">${flowers}${bird('footer')}</div></div>`;
