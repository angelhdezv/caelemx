...
Content: `data/invitations/bordado.json`, loaded through the shared service adapter. The HTML `data-source` attribute selects a different event without editing JS/CSS. The four templates now share a JSON contract and array-based schedule. See `docs/wedding-catalog.md` for replication instructions. RSVP is a simulation, not persisted and not sent to a server.

Desktop: the hero garland retains its original 3:1 proportions inside a centered width limit. Schedule entries form a horizontal itinerary; the details and illustrated attire sit beside the RSVP card below it. Mobile keeps its vertical reading order. The closing floral garland is unchanged.
...
- `assets/wedding/bordado/attire.webp`: generated formalwear illustration, dusty-rose long gown and dark-green suit, watercolor/pencil fabric details and delicate embroidery, warm ivory background, no people or text. Optimized to 960 × 640 and referenced through `dressCode.illustration`.
